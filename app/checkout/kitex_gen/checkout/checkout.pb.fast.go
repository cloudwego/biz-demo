...
func (x *PlaceOrderRes) FastRead(buf []byte, _type int8, number int32) (offset int, err error) {
	switch number {
	case 1:
		offset, err = x.fastReadField1(buf, _type)
		if err != nil {
...
SkipFieldError:
	return offset, fmt.Errorf("%T cannot parse invalid wire-format data, error: %s", x, err)
ReadFieldError:
	return offset, fmt.Errorf("%T read field %d '%s' error: %s", x, number, fieldIDToName_PlaceOrderRes[number], err)
}

func (x *PlaceOrderRes) fastReadField1(buf []byte, _type int8) (offset int, err error) {
	var v OrderResult
	offset, err = fastpb.ReadMessage(buf, _type, &v)
	if err != nil {
		return offset, err
...
func (x *PlaceOrderRes) FastWrite(buf []byte) (offset int) {
	if x == nil {
		return offset
	}
	offset += x.fastWriteField1(buf[offset:])
...
func (x *PlaceOrderRes) fastWriteField1(buf []byte) (offset int) {
	if x.Order == nil {
		return offset
	}
...
func (x *PlaceOrderRes) Size() (n int) {
	if x == nil {
		return n
	}
	n += x.sizeField1()
...
func (x *PlaceOrderRes) sizeField1() (n int) {
	if x.Order == nil {
		return n
	}
...
var fieldIDToName_PlaceOrderRes = map[int32]string{
	1: "Order",
}
